...
class OnlineFixesManager(QObject):
    ...
    def apply_fix(self, appid: int, download_url: str, install_path: str, 
                  fix_type: str = '', game_name: str = '') -> bool:
        """
        ...
        try:
            ...
            if not install_path or '..' in install_path:
                raise ValueError(f"Invalid install path (directory traversal): {install_path}")
            ...
            class FixWorkerThread(QThread):
                def __init__(self, manager, appid, download_url, install_path, fix_type, game_name):
                    super().__init__()
                ...
                def run(self):
                    try:
                        self.manager._download_and_extract_fix_worker(
                            self.appid, self.download_url, self.install_path, 
                            self.fix_type, self.game_name
                        )
                    except Exception as e:
    ...
    def _download_and_extract_fix_worker(self, appid: int, download_url: str, 
                                       install_path: str, fix_type: str, game_name: str):
        ...
        try:
            ...
            if not parent_dir or not os.path.exists(parent_dir):
                raise ValueError(f"Invalid parent directory for temp files: {parent_dir}")
            
            temp_dir = os.path.join(parent_dir, 'temp')
            temp_dir = os.path.normpath(temp_dir)
            if not temp_dir.startswith(parent_dir):
                raise ValueError(f"Path traversal attempt in temp directory: {temp_dir}")
            ...
            self._create_install_log(appid, install_path, fix_type, download_url, 
                                   game_name, extracted_files)
            ...
            # Emitir sucesso
            try:
                self.fix_download_progress.emit(100, f"{fix_type.title()} fix applied successfully!")
    ...
    def _extract_fix_zip(self, zip_path: str, install_path: str, appid: int) -> List[str]:
            ...
            with zipfile.ZipFile(zip_path, 'r') as zf:
                ...
                for name in all_names:
                    ...
                    if info.file_size > max_size:
                        raise ValueError(f"File too large in ZIP: {name} ({info.file_size} bytes)")
                ...
                if (len(top_level_entries) == 1 and 
                    appid_folder.rstrip('/') in top_level_entries):
                    
                    # Extrair conteúdo da pasta appid, não a pasta em si
                    logger.info(f"Found single folder {appid} in zip, extracting its contents")
    ...
    def _create_install_log(self, appid: int, install_path: str, fix_type: str, 
