...
class SteamlessIntegration(QObject):
    ...
    def find_game_executables(self, game_directory: str) -> List[str]:
            ...
            for file in files:
                if file.lower().endswith('.exe'):
                    ...
                    if file_size < 100 * 1024:  # < 100KB
                        logger.debug(f"Skipping executable (size filtered): {file} ({file_size} bytes)")
                        continue
                    ...
                    exe_files.append({
                        ...
                        'name': file,
                        'size': file_size,
                        'priority': self._calculate_exe_priority(file, game_name, file_size)
                    })
                else:
        ...
        logger.debug(f"Directory scan complete. Total files: {len(all_files_found)}, EXE files: {exe_count}, After filtering: {len(exe_files)}")
    ...
    def _calculate_exe_priority(self, filename: str, game_name: str, file_size: int) -> int:
        ...
        if any(word in filename_lower for word in ['editor', 'tool', 'config', 'settings']):
            priority -= 20
        
        # High penalty for crash handlers and utilities
        if any(word in filename_lower for word in ['crash', 'handler', 'debug', 'unitycrash']):
            priority -= 50
    ...
    def process_game_with_steamless(self, game_directory: str) -> bool:
        ...
        for i in range(max_attempts):
            ...
            self.progress.emit(f"Attempt {i+1}/{max_attempts}: Processing {exe_name} (priority: {priority})")
        ...
        try:
            ...
            if process.stdout:
                for line in iter(process.stdout.readline, ''):
                    ...
                    if ("steam stub" in line.lower() or "drift" in line.lower() or 
                        "steamstub" in line.lower() or "packed with" in line.lower()):
                        has_drm = True
                    
                    # Check for unpacked file creation
                    if ("unpacked file saved to disk" in line.lower() or 
                        "unpacked file saved as" in line.lower() or
                        "successfully unpacked file" in line.lower() or
                        ("unpacked" in line.lower() and ".exe" in line.lower())):
                        unpacked_created = True
            
            process.wait()
            
            if process.returncode != 0:
                self.error.emit(f"Steamless failed with exit code: {process.returncode}")
                return False
            ...
            if actual_unpacked_created:
                self.progress.emit(f"Unpacked file detected: {os.path.basename(unpacked_exe)}")
                return self._handle_unpacked_files(exe_path)
            else:
                if unpacked_created:
                    self.progress.emit("Steamless output indicated unpacked file was created, but file not found.")
                else:
                    self.progress.emit("Steamless completed but no unpacked file was created.")
                self.finished.emit(True)
                return True
    ...
    def _handle_unpacked_files(self, original_exe: str) -> bool:
        ...
        try:
            ...
            if os.path.exists(unpacked_exe):
                ...
                self.progress.emit(f"Renamed original: {os.path.basename(original_exe)} -> {os.path.basename(original_backup)}")
                
                # Rename unpacked to original name
                shutil.move(unpacked_exe, original_exe)
                self.progress.emit(f"Renamed unpacked: {os.path.basename(unpacked_exe)} -> {os.path.basename(original_exe)}")
                
                self.progress.emit("Steam DRM successfully removed!")
                self.finished.emit(True)
                return True
            else:
                self.progress.emit("Unpacked file not found. DRM may not have been present or removable.")
                self.finished.emit(True)
                return True
