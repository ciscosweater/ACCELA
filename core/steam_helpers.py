...
def find_steam_install():
    ...
    if sys.platform == 'win32':
        ...
        logger.warning(f"Automatic Steam path detection is not supported on this OS: {sys.platform}.")
        return None
...
def kill_steam_process():
    ...
    steam_proc = next((p for p in psutil.process_iter(['pid', 'name']) if p.info['name'].lower() == process_name), None)
...
def start_steam():
    ...
    try:
        if sys.platform == 'win32':
            steam_path = find_steam_install()
            if not steam_path: return 'FAILED'
            exe_path = os.path.join(steam_path, 'steam.exe')
            if not os.path.exists(exe_path): return 'FAILED'
            ...
            def launch_with_audit(so_path):
                ...
                try:
                    ...
                    if 'ELF 32-bit' in result.stdout:
                        logger.warning("SLSsteam.so is 32-bit, this may cause issues on 64-bit systems")
                except:
                    pass
...
def run_dll_injector(steam_path):
    ...
    if not os.path.exists(injector_path): return False
    try:
