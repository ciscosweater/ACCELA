...
class GameImageManager(QObject):
    ...
    def __init__(self, cache_manager: ImageCacheManager):
        ...
        self.image_formats = [
            {"name": "header", "path": "/steam/apps/{app_id}/header.jpg", "width": 460, "height": 215},
            {"name": "library", "path": "/steam/apps/{app_id}/library_600x900.jpg", "width": 600, "height": 900},
            {"name": "capsule_231x87", "path": "/steam/apps/{app_id}/capsule_231x87.jpg", "width": 231, "height": 87},
            {"name": "capsule_184x69", "path": "/steam/apps/{app_id}/capsule_184x69.jpg", "width": 184, "height": 69},
            {"name": "header_292x136", "path": "/steam/apps/{app_id}/header_292x136.jpg", "width": 292, "height": 136}
        ]
                ...
                try:
                    ...
                    if str(app_id) in data and data[str(app_id)]["success"]:
                        ...
                        if "screenshots" in app_data:
                            for screenshot in app_data["screenshots"][:2]:  # Take first 2
                        ...
                        if image_urls:
                            logger.debug(f"Found {len(image_urls)} images from API for app {app_id}")
                            return image_urls
                    
                except requests.RequestException as e:
                    logger.debug(f"API endpoint {api_endpoint} failed for app {app_id}: {e}")
                    continue
    ...
    def download_image(self, url: str, timeout: int = 10) -> Optional[bytes]:
        ...
        try:
            ...
            if len(response.content) < 1024:  # Less than 1KB
                logger.debug(f"Image from {url} is too small: {len(response.content)} bytes")
                return None
...
class GameImageThread(QThread):
        ...
        try:
            ...
            for url_info in urls:
                if url_info["format"] == self.preferred_format:
                    cached = self.manager.cache_manager.get_cached_image(self.app_id, url_info["url"])
    ...
    def _try_preferred_format(self) -> Optional[QPixmap]:
        ...
        try:
            for endpoint in self.manager.cdn_endpoints:
                format_info = next((f for f in self.manager.image_formats if f["name"] == self.preferred_format), None)
                if format_info:
                    ...
                    if image_data and self.manager.validate_image_data(image_data):
                        ...
                        if pixmap.loadFromData(image_data):
                            ...
                            self.manager.cache_manager.cache_image(self.app_id, url, image_data)
                            logger.debug(f"Downloaded preferred format for app {self.app_id}")
                            return pixmap
            return None
        except Exception as e:
    ...
    def _try_all_formats(self, primary_only: bool = False) -> Optional[QPixmap]:
        ...
        try:
            endpoints = [self.manager.cdn_endpoints[0]] if primary_only else self.manager.cdn_endpoints
            ...
            for endpoint in endpoints:
                for format_info in self.manager.image_formats:
                    ...
                    cached = self.manager.cache_manager.get_cached_image(self.app_id, url)
                    if cached and not cached.isNull():
                        logger.debug(f"Found cached {format_info['name']} for app {self.app_id}")
                        return cached
                    ...
                    if image_data and self.manager.validate_image_data(image_data):
                        ...
                        if pixmap.loadFromData(image_data):
                            # Cache the successful download
                            self.manager.cache_manager.cache_image(self.app_id, url, image_data)
                            logger.debug(f"Downloaded {format_info['name']} for app {self.app_id}")
                            return pixmap
            return None
        except Exception as e:
